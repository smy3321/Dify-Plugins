...
--no-index --find-links=./wheels/
	...
	cd ${CURR_DIR}
	local CMD_PATH="${CURR_DIR}/${CMD_NAME}"
	if [ -f "$CMD_PATH" ]; then
		chmod 755 "$CMD_PATH"
	elif command -v dify &> /dev/null; then
		CMD_PATH="dify"
	else
		echo "Error: Neither ${CMD_NAME} nor dify command found."
		exit 1
	fi
	$CMD_PATH plugin package ${CURR_DIR}/${PACKAGE_NAME} -o ${CURR_DIR}/${PACKAGE_NAME}-${PACKAGE_SUFFIX}.difypkg --max-size 5120
	if [ $? -ne 0 ]; then
